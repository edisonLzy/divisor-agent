...
import { ArrowLeft, Bot, Paintbrush, Settings } from "lucide-react";
import { type CSSProperties } from "react";
import { NavLink, Outlet, useNavigate } from "react-router-dom";

type SettingsSection = "appearance" | "models";
...
export function SettingsSectionContainer({
  ...
  return (
    <div className={cn("mx-auto flex min-h-full w-full flex-col px-10 py-12", maxWidthClass)}>
      <h1 className="mb-8 text-center text-[20px] font-medium text-foreground">{title}</h1>
      {children}
    </div>
  );
}
