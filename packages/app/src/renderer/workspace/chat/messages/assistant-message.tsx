      ...
      <Collapsible open={isProcessingOpen} onOpenChange={(open) => setIsProcessingOpen(open)}>
        <div className="flex flex-col gap-2">
          <CollapsibleTrigger className="group/trigger flex cursor-pointer items-center gap-1.5">
            <ProcessingTip />
            <ChevronRightIcon className="size-3.5 text-[#7C7C7C] transition-transform group-data-[state=open]/trigger:rotate-90 hover:text-[#9C9C9C]" />
          </CollapsibleTrigger>
          <Separator />
        </div>
...
function ProcessingTip() {
  ...
  return (
    <Shimmer as="span" animate={isLoading} className="text-xs text-[#9E9E9E]">
